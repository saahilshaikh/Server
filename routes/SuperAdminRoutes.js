...
module.exports = (app) => {
	...
	app.post("/api/controller/addResource", requireLogin, async (req, res) => {
		const { file, resourceName, topicName, fileName, subjectName, chapterName, classId } = req.body;
		Class.findOne({ _id: classId })
			.then(clas => {
				if (clas) {
					console.log(clas);
	...
	app.post('/api/createDistrictOfficer', requireLogin, requireSuperAdmin, async (req, res) => {
		...
		DistrictOfficers.findOne({ email: email })
			.then((officer) => {
				if (officer) {
		...
		DistrictOfficers.findOne({ _id: officerId })
			.then(officer => {
				if (officer) {
					...
					Users.findOneAndDelete({ email: officer.email })
						.then(re => {
							DistrictOfficers.findOneAndDelete({ _id: officerId })
								.then(resp => {
									if (resp) {
	...
	app.post("/api/createSchool", requireLogin, requireSuperAdmin, async (req, res) => {
		...
		School.findOne({ name: name, branch: branch })
			.then(school => {
				if (school) {
				...
				if (school) {
					...
					school.save()
						.then(r => {
							...
							SchoolAdmin.findOne({ _id: schoolAdminId })
								.then(schoolAdmin => {
									if (schoolAdmin) {
										...
										Users.findOneAndDelete({ email: email })
											.then(re => {
												...
												SchoolAdmin.findOneAndDelete({ _id: schoolAdminId })
													.then(resp => {
