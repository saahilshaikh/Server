					...
					Teacher.findOne({ _id: id }).then((teacher) => {
						if (teacher) {
							...
							Users.findOneAndDelete({ email: email }).then((re) => {
								...
								Teacher.findOneAndDelete({ _id: id }).then((resp) => {
							...
							.then((response) => {
								School.findOne({ _id: schoolId }).then((school) => {
									if (school) {
										...
										school.save().then((re) => {
											if (re) {
												Users.findOne({ email: pemail }).then((user) => {
													if (user) {
														Parent.findOne({ _id: user.ref_id }).then((parent) => {
															if (parent) {
				...
				school.save().then((r) => {
					Student.findOne({ _id: id }).then((student) => {
						if (student) {
							...
							Users.findOneAndDelete({ email: email }).then((re) => {
								Student.findOneAndDelete({ _id: id }).then((resp) => {
									Parent.findOne({ email: student.pemail }).then((parent) => {
										if (parent) {
											...
											if (newChildren.length > 0) {
												...
												Parent.findOneAndDelete({ _id: parent._id }).then((p) => {
													Users.findOneAndDelete({ email: parent.email }).then((s) => {
